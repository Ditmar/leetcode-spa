...
export function Badge({
  ...
  if (display === 'standalone') {
    ...
    return (
      <StandaloneBadgeLabel
        tone={resolvedTone}
        role="status"
        aria-label={
          variant === 'dot' ? 'Badge status indicator' : `Badge content: ${String(resolvedContent)}`
        }
        title={typeof resolvedContent === 'string' ? resolvedContent : undefined}
      >
        {resolvedContent}
      </StandaloneBadgeLabel>
    );
