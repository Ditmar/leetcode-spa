...
const lineSeries = [
  { data: [100, 150, 130, 200, 170], label: 'Visitors' },
];
...
describe('Chart — bar', () => {
  ...
  it('has role="img" for accessibility', () => {
    const { getByRole } = render(
      <Chart type="bar" series={barSeries} title="Revenue chart" />,
    );
  ...
  it('renders accessible title', () => {
    const { getByText } = render(
      <Chart type="bar" series={barSeries} title="Monthly Revenue" />,
    );
...
describe('Chart — empty data', () => {
  it('renders bar chart with empty series array without crashing', () => {
    const { getByTestId } = render(
      <Chart type="bar" series={[]} title="Empty chart" />,
    );
    expect(getByTestId('chart')).toBeTruthy();
  });
  ...
  it('renders pie chart with no data values without crashing', () => {
    const { getByTestId } = render(
      <Chart type="pie" series={[{ data: [] }]} title="Empty pie" />,
    );
    expect(getByTestId('chart')).toBeTruthy();
  });
});
